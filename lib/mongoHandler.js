...
/**
   User password store
*/
MongoStore.prototype.userUpdate = function (user, pass, callback) {
  var self = this
  if(!user || !pass) return
  var col = self._db.collection('person')
  col.update({user: user})
}
...
MongoStore.prototype.getInheritedTable = function (colName) {
  // get inherited reports etc.
  const inheritedTable = {}
  for(let key in this._formtype) {
    let t = this._formtype[key]
    if('form_' + t.inherited==colName) {
      inheritedTable[key] = {
        type: t.formType,
        title: t.title
      }
...
MongoStore.prototype.search = function (request, callback) {
  var self = this
  var _resources = self.resourceConfig._resources
  // console.log(reqType, inheritedType,333)
  var inheritedType = request.inherited
  if(inheritedType) {
    var reqFormType  = self._formtype[inheritedType]
  }
  var colName = request.params.type
  var qs = querystring.parse(request.route.query)
  // console.log(inheritedType, colName, request.params.type, 333)
  ...
  if (formType) {
    ...
    const targetForm = reqFormType || formType
    if(targetForm){
      for(var i in targetForm.templateRef) {
        const attr = targetForm.templateRef[i].attrs
        const inherited = attr && attr.inherited
        if (inherited) {
          inheritedObj[inherited] = {
            name: i,
            joiType: attributes[i],
            dom: targetForm.template[i],
            ref: targetForm.templateRef[i]
          }
  ...
  async.parallel({
    ...
    // err, result, totalrows, meta
    if (/GET/i.test(request.route.verb)) {

      // do sth with formtypes....
      // dom is really large, don't transfer it!!
      for(let f in tableLookups) {
        // if(tableLookups[f] && tableLookups[f].dom) delete tableLookups[f].dom
        if(tableLookups[f]) tableLookups[f].__reports__ = self.getInheritedTable(f)
      }
      if(colName == 'formtype') {
        results.resultSet.forEach(v=>{
          // if(v && v.dom) delete v.dom
          if(v) v.__reports__ = self.getInheritedTable('form_' + v.name)
        })
      }
      ...
      // console.log(request.route, qs, qs.joitype, 333)
      if(qs.joitype) {
        meta.joitype = {
          '': attributes
        }
    ...
    return callback(err, results.resultSet, results.totalRows, meta)
  })
}
...
MongoStore.prototype.delete = co.wrap(function* (request, callback) {
  ...
  var collection = this._db.collection(request.params.type)
  var documentId = MongoStore._mongoUuid(request.params.id)
  var backupCol = this._db.collection('backup')
  var user = request.user
  collection.findOne({ _id: documentId }, function(err, oldDoc) {
    backupCol.insert({action: 'deleteOne', user: user.name, col: request.params.type, date: new Date(), data: oldDoc})
...
MongoStore.prototype.update = co.wrap(function* (request, partialResource, callback) {
  ...
  // console.log('findOneAndUpdate', JSON.stringify(partialDocument))
  var backupCol = this._db.collection('backup')
  var user = request.user
  collection.findOne({_id: documentId}, {fields: { _id: 0 }}, function (e, oldDoc) {
    if(e) return callback(e)
    backupCol.insert({action: 'updateOne', user: user.name, col: request.params.type, date: new Date(), data: oldDoc})
    var op = {
      $set: partialDocument
    }
    var meta = partialDocument.meta
    if(meta && meta.__unset__){
      op.$unset = {}
      meta.__unset__.forEach(function(val) {
        op.$unset[val] = 1
      })
      delete meta.__unset__
    }
