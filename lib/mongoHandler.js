...
MongoStore.prototype.search = function (request, callback) {
  ...
  var reqFormType = self._formtype[reqType]
  if(reqFormType){
    var inheritedType  = reqFormType.inherited
    if(inheritedType) {
      inheritedType = 'form_' + inheritedType
      request.base = inheritedType
    }
  }
  // console.log(reqType, inheritedType,333)
  var colName = inheritedType || reqType
  ...
  if (formType) {
    ...
    var tkeyArray = request.params.noextra || inheritedType ? [] : helper.getTkey(formType)
    // populate all inherited fields
    if(reqFormType){
      for(var i in reqFormType.templateRef) {
        const attr = reqFormType.templateRef[i].attrs
        const inherited = attr && attr.inherited
        if (inherited) {
          inheritedObj[inherited] = {
            name: i,
            joiType: attributes[i],
            dom: reqFormType.template[i],
            ref: reqFormType.templateRef[i]
          }
