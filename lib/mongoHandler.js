...
MongoStore.prototype._getSearchCriteria = function(request) {
  ...
  var criteria = Object.keys(request.params.filter).map(function(attribute) {
    ...
    var castValue=function(v, start) {
      return joi.attempt(v.substring(start||0), attributeConfig)
    }
...
MongoStore.prototype._applySort = function(request, cursor) {
  ...
  var attributes = this.resourceConfig.attributes
  ...
  ;[].concat(qs.sort).forEach(attribute => {
    ...
    // external keys: lahuojihua.adate
    // will be come: __lahuojihua.adate
    if(attribute.indexOf('.')>0) attribute = '__' + attribute

    // it's tkey external keys: charger
    const settings = attributes[attribute]._settings
    if(settings) {
      const relation = settings.__one || settings.__many
      const tkey = settings.__tkey
      if(relation && tkey) {
...
MongoStore.prototype.search = function(request, callback) {
  ...
  var filterp = request.params.filterp || {}
  if(parent) {
    ...
    if (condition) {
      Object.keys(condition).forEach(v=>{
        if (condition[v][0]==':') {
          // fizzy match
          matchp[v] = {$regex : condition[v].substr(1), $options: 'i'}
        } else {
          matchp[v] = condition[v]
        }
      })
    ...
    if(settings) {
      ...
      if(relation && tkey) {
        ...
        if (filter) {
          if (filter[0] == ':') {
            // fizzy match
            matchp[tkey] = {$regex: filter.substr(1), $options: 'i'}
          } else {
            matchp[tkey] = filter
          }
  ...
  console.log(JSON.stringify(query), 1234)
