...
var {getTkey, str2Query, joiCastValue, getFormTree} = require('./helper')
...
MongoStore.prototype.search = function(request, callback) {
  ...
  if(tkeyArray && tkeyArray.length) {
    tkeyArray.forEach(v=>{
      ...
      const tree = getFormTree(segment, colName, _resources)
      if (tree.length == segment.length && tree[tree.length-1].err) {
