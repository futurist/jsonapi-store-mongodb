...
MongoStore.prototype.search = function (request, callback) {
  ...
  var exitFormTypes = request.safeHeaders['exist-formtypes'].split(',')
  ...
  if (formType) {
    ...
    var tkeyArray = helper.getTkey(formType)
  }
  try {
  ...
  if (tkeyArray && tkeyArray.length) {
    ...
    tkeyArray.forEach(v => {
      ...
      if (tree.length == segment.length) {
        ...
        fields.forEach(v=>{
          // extraField[segment.concat(v).join('/')] = helper.last(tree).form.attributes
          extraField.push({
            key: '__' + resolvedFields.join('/'),
            field: v,
            table: helper.last(forms),
            path: segment.concat(v).join('/')
          })
        })
      }
    })
    ...
    _query2.forEach(v => {
      ...
      if(exitFormTypes.indexOf(v.from)<0) tableLookups[v.from] = self._formtype[v.from]
    })
  }
    ...
    if (settings) {
      ...
      if (relation && tkey) {
        ...
        if (!existQuery) {
          ...
          if(exitFormTypes.indexOf(relation)<0) tableLookups[relation] = self._formtype[relation]
        }
        ...
        if (filter) {
          if (filter[0] == ':') {
            // fizzy match
            matchp[tkey] = {$regex: helper.joiCastValue(filter, 1, attributes[tkey]), $options: 'i'}
          } else {
            matchp[tkey] = helper.joiCastValue(filter, 0, attributes[tkey])
          }
  ...
  console.log(JSON.stringify(query), request.params.filter, request.params.sort, request.cookies, 1234)
