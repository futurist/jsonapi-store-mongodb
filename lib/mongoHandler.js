...
MongoStore.prototype._getSearchCriteria = function(request) {
  ...
  var criteria = Object.keys(request.params.filter).map(function(attribute) {
    ...
    var castValue=function(v, start) {
      const val = joi.attempt(v.substring(start||0), attributeConfig)
      return val
    }
...
function castValue (v, start, attributeConfig) {
  const str = v.substring(start||0)
  const val = attributeConfig ? joi.attempt(str, attributeConfig) : str
  return val
}
...
MongoStore.prototype.search = function(request, callback) {
  ...
  if(parent) {
    const externalTypeDefs = _resources['form_' + parent]
    const attributes = externalTypeDefs ? _resources['form_' + parent].attributes : {}
    ...
    if (condition) {
      Object.keys(condition).forEach(v=>{
        if (condition[v][0]==':') {
          // fizzy match
          matchp[v] = {$regex :  castValue(condition[v], 1, attributes[v]), $options: 'i'}
        } else {
          matchp[v] = castValue(condition[v], 0, attributes[v])
        }
      })
    ...
    if(settings) {
      ...
      if(relation && tkey) {
        ...
        if(!existQuery) {
          query.push(
            // result name is __lahuojihua, __ indicate foreign
            {$lookup: {from: 'form_' + relation, localField: attr+'.id', foreignField: 'id', as: '__' + relation }}
          )
        }

        const externalTypeDefs = _resources['form_' + relation]
        ...
        if (filter) {
          if (filter[0] == ':') {
            // fizzy match
            matchp[tkey] = {$regex: castValue(filter, 1, attributes[tkey]), $options: 'i'}
          } else {
            matchp[tkey] = castValue(filter, 0, attributes[tkey])
          }
