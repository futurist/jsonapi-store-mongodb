...
var {addToSet, buildQueryFromTree, getTkey, str2Query, joiCastValue, getFormTree} = require('./helper')
...
MongoStore.prototype._getSearchCriteria = function (request) {
  ...
  var criteria = Object.keys(request.params.filter).map(function (attribute) {
    ...
    values = values.map(val => {
      return str2Query(val, attributeConfig)
    }).map(function (qval) {
      var tmp = { }
      tmp[attribute] = qval
      return tmp
...
MongoStore.prototype._applySort = function (request, cursor) {
  ...
  ;[].concat(qs.sort).forEach(attribute => {
    ...
    if (attrArr.length > 1) {
      // external parent keys: lahuojihua.adate
      // will be come: __lahuojihua.adate
      if (attrArr[0] == parent) {
        // it's parent external key
        attribute = '__' + attribute
      } else {
        // it's not included in parent
      }
    } else {
...
MongoStore.prototype.search = function (request, callback) {
  ...
  // console.log('**query: ', util.inspect(query.params, {depth:null}))
  // console.log(getFormTree('~/inputText5/name'.split('/'), 'form_lahuojihua_list', _resources))
  if (formType) {
    var parentTable = formType.table
    var tkeyArray = getTkey(formType)
  }
  try {
  ...
  const relationAlias = {}
  const parent = request.params.parent
  const filterp = request.params.filterp || {}
  ...
  if (tkeyArray && tkeyArray.length) {
    ...
    tkeyArray.forEach(v => {
      ...
      const tree = getFormTree(segment.slice(0, -1), colName, _resources)
      // last segment always field name, not formtype
      // console.log(tree.length, segment.length)
      if (tree.length == segment.length - 1) {
        buildQueryFromTree(tree).forEach(v => {
      ...
      if (relation && tkey) {
        ...
        if (!existQuery) {
          query.push(
            ...
            {$lookup: {from: relation, localField: attr + '.id', foreignField: 'id', as: '__' + relation }}
          )
          addToSet(tableLookups, relation)
        }
        ...
        if (filter) {
          if (filter[0] == ':') {
            // fizzy match
            matchp[tkey] = {$regex: joiCastValue(filter, 1, attributes[tkey]), $options: 'i'}
          } else {
            matchp[tkey] = joiCastValue(filter, 0, attributes[tkey])
          }

          // remove existing match criteria
          delete request.params.filter[attr + '.id']
          query.push({$match: { ['__' + relation]: {$elemMatch: matchp} } })
        }
      }
    }
  })

  console.log(JSON.stringify(query), request.params.filter, request.params.sort, 1234)
