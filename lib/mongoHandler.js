...
MongoStore.prototype.search = function (request, callback) {
  ...
  if (tkeyArray && tkeyArray.length) {
    ...
    _query2.forEach(v => {
      query.push(
        { '$lookup': v },
        { '$unwind': {path: '$' + v.as, includeArrayIndex: v.as + '_arrayIndex', preserveNullAndEmptyArrays: true }}
      )
      ...
      if (relation && tkey) {
        ...
        if (!existQuery) {
          ...
          query.push(
            ...
            {"$unwind":{path: '$' + as, includeArrayIndex: as + '_arrayIndex', preserveNullAndEmptyArrays: true}}
          )
