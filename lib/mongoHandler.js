...
MongoStore.prototype.search = function(request, callback) {
  ...
  console.log(JSON.stringify(criteria), request.route.query, request.params, self.relationshipAttributeNames, 1234)
  ...
  // get filter foreign key
  var filterForeign = request.params.filterf
  filterForeign && Object.keys(filterForeign).forEach(function(v) {
    // parent.key form become __parent.key
    if(v.indexOf('.')>0) filterForeign[v] = '__' + filterForeign[v]
  })
  delete request.params.filterf
  console.log(filterForeign)
  if(filterForeign) query.push({$match: filterForeign})

  var parent = request.params.parent
  if(parent) {
    query.push(
      // result name is __lahuojihua, __ indicate foreign
      {$lookup: {from: 'form_' + parent, localField: "parentID.id", foreignField: "id", as: "__" + parent}}
    )
  }
