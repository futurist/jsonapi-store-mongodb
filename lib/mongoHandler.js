...
MongoStore.prototype.search = function (request, callback) {
  ...
  if (tkeyArray && tkeyArray.length) {
    ...
    tkeyArray.forEach(v => {
      ...
      const tree = helper.getFormTree(segment.slice(0, -1), colName, _resources)
    ...
    _query2.forEach(v => {
      ...
      tableLookups[v.from] = self._formtype[v.from]
    })
  }
    ...
    if (settings) {
      ...
      if (relation && tkey) {
        ...
        if (!existQuery) {
          ...
          tableLookups[relation] = self._formtype[relation]
        }
